...
func (s *MsgRepoHistoryAPISuite) getProblemMessagesWhileCursor(
	problemID types.ProblemID,
	pageSize int,
) ([][]msg, []cursor) {
	s.T().Helper()
	...
	// get all messages and cursors while cursor not nil and return its
	for {
		msgs, next, err = s.repo.GetProblemMessages(s.Ctx, problemID, pageSize, next)
		s.Require().NoError(err)
		result = append(result, apply(msgs, newMsgFromRepoMsg))
